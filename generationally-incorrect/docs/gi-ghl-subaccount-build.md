...
- **Email sending domain should be a `generationallyincorrect.*` domain, not `ninja360.net`.** Update (Sep 12): `generationallyincorrect.net` — Tim's own, registered and live, separate from Erik's untouched `.com` — now has SPF and DKIM in place; **DMARC is still missing.** No sends until that's done, which is why Section 6 still deliberately omits a contact-facing email step, including the eventual "welcome" send this spec has always parked behind this blocker.
- **Do not email the borrowed 5,000-name realtor list.** Called out explicitly as a blocker, not a suggestion — that list is on loan, not opted in to anything GI-related. Nothing in this spec sends to it and nothing should, regardless of how tempting a launch-announcement blast looks once Section 6's workflow is live.
- **Live signups are landing in the wrong sub-account.** A GHL-side audit (Sep 12) found the retired form `opZvykeXf7B6XOkP3wc5` still exists — and is still collecting submissions — inside the **Ninja-360** agency sub-account, not here. 4 submissions, most recent Sep 5 (one named "Qr Direct" / "test of qr code"), got no tag, no opportunity, no notification, because that sub-account has none of Sections 2–6 behind it. This repo's own code is clean — zero references to that form ID anywhere after the Sep 12 permanent-form-ID swap — so the leak is an external surface, most likely a QR code or link built by pointing straight at the raw `links.ninja360.net/widget/form/<id>` URL instead of at the site (`?src=` pattern, Section 9), which is exactly the failure mode Section 9's "GHL native, not static image" guidance exists to prevent: the destination can be repointed in one place without reprinting anything, but only if the thing being printed points at the *site*, not a raw form URL. **Standing rule going forward: every new GHL build for this show — forms, workflows, pipelines, custom fields, tags — goes in the Generationally Incorrect sub-account (`whkhRNZwHhdpUToE7SpN`), never Ninja-360's (`cCPr3A6gnc4ihe1G44w5`).** Whoever owns QR/link distribution still needs to find and repoint whatever's driving traffic to the old ID.
